...
class Command(BaseCommand):
    """
    Класс, описывающий команду для генерации уникальных кодов менеджеров
    и сохранения их в базе данных.

    Эта команда создаёт заданное количество уникальных кодов определённой
    длины и сохраняет их в модель ManagerCode. Каждый код состоит из заглавных
    ...
    Методы:
        - add_arguments(parser):
            Добавляет аргументы (количество кодов, длина кодов) командной
            строки для управления генерацией кодов.
    ...
    Аргументы командной строки:
        ...
        - --length (int): Необязательный аргумент. Длина каждого кода
    ...
    def add_arguments(self, parser) -> None:
        parser.add_argument(
            "count", type=int, help="Количество кодов для генерации"
        )
        parser.add_argument(
            "--length",
            type=int,
